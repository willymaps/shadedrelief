...
harrison-hawaii:=https://www.shadedreliefarchive.com/download/Hawaii_Harrison.zip
...
proj-hawaii:=+proj=aea +lon_0=-157 +lat_1=8 +lat_2=18 +lat_0=13 +datum=NAD83 +no_defs
...
download:
	...
	curl $(harrison-hawaii) -o tmp/harrison-hawaii.zip ; \
	unzip -o tmp/harrison-hawaii.zip -d tmp/ ; \
	rm -f tmp/harrison-hawaii.zip ; \
...
process:
	...
	gdalwarp -s_srs '${proj-hawaii}' -t_srs EPSG:4326 tmp/Hawaii_Harrison/hawaii_harrison.tif tmp/harrison_hawaii.tif -dstalpha -overwrite -q ; \
...
image:
	...
	gdalwarp -t_srs EPSG:3857 -ot Byte -of png tmp/harrison_hawaii.tif images/harrisonhawaii.png -dstalpha -overwrite -q --config GDAL_PAM_ENABLED false ; \
...
compress:
	...
	mogrify -format png -resize 100% images/harrisonhawaii.png ; \
...
tile:
	...
	rio pmtiles tmp/harrison.tif tmp/harrisonhawaii.pmtiles --format PNG --zoom-levels 0..8 --resampling bilinear ; \
